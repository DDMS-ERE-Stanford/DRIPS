close all;
clear all;
% function e_pod = AdvDiff_POD_multiP(u_test,kappa_test,ybar_test)
dt = 0.01;
t = 0:dt:1;
N_t = length(t);
...
% visualize(vec_to_imag(w_test,bc_test),'HFM');
...
% wr_train_dmd = zeros(N_w,N_t,N_MC);
% ROB_train_dmd = zeros(N_w,N_q,N_MC);
% Kr_train_dmd = zeros(N_q,N_q,N_MC);
% Br_train_dmd = zeros(N_q,N_MC);
...
for i1 = 1:size_p1(1)
    for i2 = 1:size_p1(2)
        for i3 = 1:size_p1(3)
            ...
            Kr_train(:,:,i1,i2,i3) = ROB_train(:,:,i1,i2,i3)'*K*ROB_train(:,:,i1,i2,i3);
            Br_train(:,i1,i2,i3) = ROB_train(:,:,i1,i2,i3)'*B;
        end
    end
end
...
[ROB_test_interp_general,Kr_test_interp,Br_test_interp] = interpolate_PROM...
    (ROB_train,ROB_test_interp,Kr_train,Br_train,i0,p,u_test,kappa_test,ybar_test,'spline');
wr_test_interpPROM = ROM_construction(w_test,ROB_test_interp_general,...
    Kr_test_interp,Br_test_interp,N_t);
toc
% visualize(vec_to_imag(wr_test_interpPROM,bc_test),'interpolated PROM');
% visualize(vec_to_imag(abs(w_test-wr_test_interpPROM),zeros(size(bc_test))),'interpolated PROM error');
e_pod = norm(w_test-wr_test_interpPROM)./norm(w_test);
% end
...
function visualize(T,titletext)
    ...
    ylabel('$y$','FontUnits','points','interpreter','latex',...
     'FontSize',10);
     title('$t=0.5$','FontUnits','points','interpreter','latex',...
     'FontSize',10);
     caxis(lim);
    hcb = colorbar;
    title(hcb,{'$s$'},'FontUnits','points','interpreter','latex',...
