...
%Parameters for scenario (Modify these) ###############################################################################################################################################################################
    %set information about domain of simulation@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
        SCENARIO='scenario_sphere.png'; %<--- file (image) that is scenario for navier stokes fluid simulation
...
% DETERMINE MEMORY USAGE$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
    if exist([pwd '/temporary_NS_velocity'],'dir')==0 %if directory does not exist (external save of u and v)
        mkdir([pwd '/temporary_NS_velocity']) %create directory to store u and v
    end
    save([pwd '/temporary_NS_velocity/' '00_TEMPORARY_FILES_4_NAV_STK_CODE' '.mat' ],'XI') %save random file to directory  (testing)
    ...
    dirr=[pwd '/temporary_NS_velocity/']; %record directory path for print (below)
...
% clear all %clear all variables in workspace
% clc %clear command window
VX = load('C:/Users/hannahlu/Dropbox/Stanford/Research/dmd_parameterzied_ds/Code/Navier-stokes-2D-numerical-solve-incompressible-flow-with-custom-scenarios-MATLAB--master/data_generator/temporary_NS_velocity/NSTOKES_TEMP_vx_12.mat');
VY = load('C:/Users/hannahlu/Dropbox/Stanford/Research/dmd_parameterzied_ds/Code/Navier-stokes-2D-numerical-solve-incompressible-flow-with-custom-scenarios-MATLAB--master/data_generator/temporary_NS_velocity/NSTOKES_TEMP_vy_12.mat');
U = sqrt((VX.var).^2+(VY.var).^2);
U = reshape(U,[20000,250]);
toc
end
