close all;
clear all;
% function e_dmd = linear_stanford(c_test,sL_test,sR_test)
N_t= 500;
...
[w_test,qx_test,qy_test] = stanford(c_test,sL_test,sR_test,t);
% visualize(w_test,qx_test,qy_test);
...
qx_train = zeros([N_w,N_t,size_p1]);
qy_train = zeros([N_w,N_t,size_p1]);
...
for i1 = 1:size_p1(1)
    for i2 = 1:size_p1(2)
        for i3 = 1:size_p1(3)
            [w_train(:,:,i1,i2,i3),qx_train(:,:,i1,i2,i3),qy_train(:,:,i1,i2,i3)]... 
                = stanford(c(i1),sL(i2),sR(i3),t);
...
toc
% visualize(wr_test_interpPROM_dmd,0*qx_test,0*qy_test);
visualize(log(abs(w_test-wr_test_interpPROM_dmd)),0*qx_test,0*qy_test);
...
function [T,qx,qy] = stanford(s_h,sL,sR,tlist)
